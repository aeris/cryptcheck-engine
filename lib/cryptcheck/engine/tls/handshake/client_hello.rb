		...
		class Handshake
			class ClientHello
				...
				def self.read(socket, *args, **kwargs)
					...
					random = socket.recvmsg 32, *args, **kwargs

					length  = socket.recv_uint8 *args, **kwargs
					session = socket.recvmsg length, *args, **kwargs

					ciphers      = read_ciphers socket, *args, **kwargs
					compressions = read_compressions socket, *args, **kwargs
					extensions   = read_extensions socket, *args, **kwargs

					# ap version:    version, random: random, session: session,
					#    ciphers:    ciphers, compressions: compressions,
				...
				def self.read_ciphers(socket, *args, **kwargs)
					length = socket.recv_uint16 *args, **kwargs
					socket.collect length do
						tmp    = socket.recv_uint16 *args, **kwargs
						cipher = CIPHERS[tmp]
				...
				def self.read_compressions(socket, *args, **kwargs)
					length = socket.recv_uint8 *args, **kwargs
					socket.collect length do
				...
				def self.read_extensions(socket, *args, **kwargs)
					length = socket.recv_uint16 *args, **kwargs
					socket.collect length do
						extension = Extension.read socket, *args, **kwargs
						[extension.size + 4, extension]
					end
				end
			end
		end
